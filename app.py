...
# Add any other QuickBooks "Payroll Items" and their matching L&I Trade Codes here!
# -------------------------------------------------------------------
TRADE_MAPPING = {
    "insulation sn": "RESI",
    "insulation": "RESI",
...
def parse_pdf_to_workbook(uploaded_file) -> dict[str, pd.DataFrame]:
    ...
    while i < len(lines):
        ...
        if re.match(r"^\d+\.\d{2}$", line):
            ...
            # Swapped First and Last name assignment based on your feedback
            first_name = context[0] if len(context) > 0 else "Unknown"
            last_name = context[1] if len(context) > 1 else "Worker"

            # Check for Trade Mappings
            trade_code = "RESE" # Default fallback
    ...
    for idx, entry in enumerate(parsed_entries, start=1):
        ...
        # Put extracted hours into Monday (Day 1 for this period)
        for d in range(1, 8):
...
def build_wapwcpr_xml(all_sheets: dict[str, pd.DataFrame]) -> bytes:
    ...
    if "Employee ID" in emp_df.columns:
        emp_df = emp_df[emp_df["Employee ID"].notna() & (emp_df["Employee ID"].astype(str).str.strip() != "")]

    if "Trade" in trades_df.columns:
        trades_df = trades_df[trades_df["Trade"].astype(str).str.strip().str.isalpha()]
    ...
    for _, emp in emp_df.iterrows():
        ...
        etree.SubElement(emp_node, "firstName").text = str(emp.get("First Name", "")).strip()
        ...
        etree.SubElement(emp_node, "lastName").text = str(emp.get("Last Name", "")).strip()
...
# -------------------------------------------------------------------
# UI: TWO-STEP WORKFLOW TABS
# -------------------------------------------------------------------
tab1, tab2 = st.tabs(["1️⃣ Convert Timesheet to Spreadsheet", "2️⃣ Validate Spreadsheet & Generate XML"])

with tab1:
    st.header("Step 1: Parse PDF to Spreadsheet")
    st.write("Upload your timesheet PDF. The tool will extract hours, flip names to the correct order, and map trades like 'Insulation SN' to 'RESI'.")
    
    pdf_file = st.file_uploader("Upload Timesheet PDF", type=["pdf"])
    
    if pdf_file:
        parsed_sheets = parse_pdf_to_workbook(pdf_file)
        st.success("✅ Timesheet parsed successfully!")
        
        # Show a quick preview of what was found
        st.write("👀 **Quick Preview (Employees found):**")
        st.dataframe(parsed_sheets["Employees"][["First Name", "Last Name", "Employee ID"]], use_container_width=True)
        st.write("👀 **Quick Preview (Trades found):**")
        st.dataframe(parsed_sheets["Trades"][["Employee ID", "Trade", "Reg Day 1 Hours"]], use_container_width=True)

        # Generate downloadable Excel file
        output_excel = io.BytesIO()
        with pd.ExcelWriter(output_excel, engine="openpyxl") as writer:
            for s_name, s_df in parsed_sheets.items():
                s_df.to_excel(writer, sheet_name=s_name, index=False)
        output_excel.seek(0)

        st.download_button(
            label="📥 Download Spreadsheet for Editing (.xlsx)",
            data=output_excel,
            file_name="certified_payroll_template.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        )

with tab2:
    st.header("Step 2: Validate Data and Generate L&I XML")
    st.write("Once you've filled in rates, SSNs, and addresses locally in Excel, upload that final `.xlsx` file here.")
    
    excel_file = st.file_uploader("Upload Edited Excel Spreadsheet", type=["xlsx"])
    
    if excel_file:
        final_sheets = pd.read_excel(excel_file, sheet_name=None)
        st.success("✅ Excel workbook loaded successfully!")
        
        if st.button("Generate & Validate L&I XML", type="primary"):
            try:
                xml_bytes = build_wapwcpr_xml(final_sheets)
                ...
                if is_valid:
                    ...
                    st.error("❌ XML Validation Failed! Fix these in your Excel file and re-upload:")
